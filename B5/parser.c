...
void compileProgram(void) {
  ...
  eat(KW_PROGRAM);
  eat(TK_IDENT);
  Object* obj = createProgramObject(currentToken->string);
  eat(SB_SEMICOLON);
...
void compileParam(void) {
  ...
  case TK_IDENT:
    eat(TK_IDENT);
    obj = createParameterObject(currentToken->string, PARAM_VALUE, NULL);
    eat(SB_COLON);
  ...
  case KW_VAR:
    eat(KW_VAR);
    eat(TK_IDENT);
    obj = createParameterObject(currentToken->string, PARAM_REFERENCE, NULL);
    eat(SB_COLON);
    obj->paramAttrs->type = compileBasicType();
    declareObject(obj);
    break;
